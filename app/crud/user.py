...
async def retrieve_user_data(request: Request, user_id: UUID) -> dict:
  user = await request.app.db[user_collection].find_one({
    "user_id": user_id},
...
async def retrieve_users_data(request: Request) -> list:
  users = []
  async for user in request.app.db[user_collection].find(projection={"_id": False}):
    users.append(user)
...
async def create_user_data(request: Request, user: User) -> dict:
  if await request.app.db[user_collection].find_one({
    "email": user.email},
    projection={"_id": False},
  ):
  ...
  new_user_id = uuid4()
  new_user = await request.app.db[user_collection].insert_one({
    **user.dict(),
    "user_id": new_user_id,
  })
...
async def update_user_data(request: Request, user_id: UUID, user: UserInUpdate):
  updated_user = await request.app.db[user_collection].find_one_and_update(
    {"user_id": user_id},
...
async def delete_user_data(request: Request, user_id: UUID):
  deleted_user = await request.app.db[user_collection].find_one_and_delete(
    {"user_id": user_id},
    projection={"_id": False},
  )
